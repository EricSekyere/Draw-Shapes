 ...
 *                    Assignment #5
 *
 *  This program provides the View and Controller
 *  portions of the Drawing Object program.
 *
 *****************************************************/

class Draw extends JFrame implements ActionListener, MouseInputListener
{
   public static void main(String[] args)
   {
      ...
      /*
       * this is the standard GUI progrm startup code, we
       * instantiate and configure a JFrame-based object
       */
   ...
   /*
    * How close (in pixels) does one have to click to a line
    * or point in order to select it
    */
   ...
   public Draw()
   {
      ...
      leftPanel.add(new JLabel("Click and drag to create new drawing object..."));
      leftPanel.add(vp);
      ...
      /*
       * once we've placed our GUI components in a JPanel, we make
       * that panel the 'content pane' of the JFrame
       */
   ...
   // button is first pressed, suring a mouse action
   int start_of_drag_x;
   int start_of_drag_y;
   ...
   public void mousePressed(MouseEvent me)
   {
      ...
      if(previouslySelected != null && previouslySelected == currentlySelected)
      {
         ...
         isResize = ((bb.x+bb.width-SELECTION_DISTANCE < start_of_drag_x)
                   ...
                   &&(bb.y+bb.height-SELECTION_DISTANCE < start_of_drag_y)
                   &&(start_of_drag_y < bb.y+bb.height+SELECTION_DISTANCE));
         if(isResize)
         {
   ...
   /*
    * This event listener is called when the user drags the mouse button
    * (i.e., when the mouse is moved while the button is being pressed).
    */
   public void mouseDragged(MouseEvent me)
   {
      ...
      if(!isClick)
      {
         ...
         if(vp.getSelectedObject() == null)
         {
            ...
            if(line.isSelected())
               newObject = new DrawingLine(start_of_drag_x, start_of_drag_y, x, y);

            else if(square.isSelected())
               newObject = new DrawingSquare(start_of_drag_x, start_of_drag_y, x, y);

            else if(ex.isSelected())
               newObject = new DrawingX(start_of_drag_x, start_of_drag_y, x, y);

            else if(ellipse.isSelected())
               newObject = new DrawingEllipse(start_of_drag_x, start_of_drag_y, x, y);
         ...
         else
         {
            ...
            else
            {
               ...
               vp.updateSelection(x+offset_x, y+offset_y, x+bb.width+offset_x,
                                    y+bb.height+offset_y);
            }
         }
      }
   }
   ...
   /***************************************
    *
    *  This inner class contains both the
   ...
   class ViewPanel extends JPanel
   {
      ...
      /***************************************
       *        This is the Model
       *  (the data used to draw the panel)
       ***************************************/
       ...
       *
       * Its purpose is to determine which object the user
       * has clicked on.  This is accomplished by asking each
       * object whether they've been hit (the wasClicked() function).
       *
       * This function also returns a boolean which indicates
       * whether the click was in the resize corner (the lower
       * left corner)
       */
      public void processSelectionClick(int x, int y)
      {
         ...
         for(DrawingObject o : objects)
         {
            ...
            isResize = ((bb.x+bb.width-SELECTION_DISTANCE < x)
                      ...
                      &&(bb.y+bb.height-SELECTION_DISTANCE < y)
                      &&(y < bb.y+bb.height+SELECTION_DISTANCE));
      ...
      /*
       * This function returns a reference to the object that
       * the user has selected, or null if nothing is selected.
       */
      ...
      /*
       * This function unselects the object that the user had
       * previously selected.
       */
      ...
      /*************************************
       *         This is the View
       *  (the code that draws the panel
       *  contents, based upon the model)
       *************************************/
      ...
      public void paintComponent(Graphics g)
      {
         ...
         for(DrawingObject o : objects)
         {
            ...
            if(o == selectedObject)
            {
               ...
               g2.fillRect(bb.x+bb.width-SELECTION_DISTANCE,
                           bb.y+bb.height-SELECTION_DISTANCE,
                           2*SELECTION_DISTANCE,
                           2*SELECTION_DISTANCE);
               if(bb.width < 0)
               {
                  bb.x = bb.x + bb.width;
